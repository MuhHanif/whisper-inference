...
def split_stereo_audio_ffmpeg(input_file: str) -> Dict[str, str]:
    ...
    command = ['ffmpeg', '-y', '-i', input_file, '-map_channel', '0.0.0', left_file,
...
def whisper_inference(model, audio_file: str) -> Tuple[List[str], float]:
    ...
    combined_transcription = combined_transcription.sort_values(by=["start", "seek", "channel"])
...
def whisper_to_vtt(model, audio_file_path: str, save_output_as_file: bool = True, output_dir: str = "/tmp/whisper_temp") -> Dict[str, str]:
    """
